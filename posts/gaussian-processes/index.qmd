...
I stumbled upon Gaussian Processes when looking into how the MIPRO prompt optimisation algorithm works, and it is really fascinating. It's a very nifty application of Bayesian modelling to analyse functions where we are low on data points.
...
2. You can use the samples to answer questions like "what's the probability that $f(0) > 0$" or "what's the probability that the maximum sits between $f(-1)$ and $f(1)$".
