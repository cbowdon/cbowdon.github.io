---
title: "Latent F1 constructor performance in 2024: fun with Bayesian models"
author: "Chris Bowdon"
date: "2024-12-27"
...
f1data <- races |>
  ...
  merge(drivers) |>
  merge(circuits) |>
  select(raceId, gp, year, round, circuitRef, constructorRef, driverRef, q1, q2, q3, quali_position, grid, position, team_colour) |>
...
ggplot(f2024) +
  ...
  scale_fill_manual(values = team_colours$team_colour) +
...
ggplot(merge(ctr_positions, team_colours)) +
  aes(x = position, fill = constructorRef) +
...
plot_posterior <- function(fit) {
  ggplot(sample_posterior(fit)) +
    ...
    facet_wrap(~constructorRef) +
    scale_fill_manual(values = team_colours$team_colour) +
    geom_density(alpha = 0.5, colour = "black", linewidth = 0.1, show.legend = FALSE)
}
...
Round three is clearly a balls-up: how can Perez' 5th place become the 10th constructor position? Let's work through it again.
