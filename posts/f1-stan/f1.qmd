...
position_gaps <- races |>
  ...
  merge(results) |>
  merge(circuits2024) |>
  select(year, round, circuitRef, constructorId, driverId, position) |>
  filter(position != "\\N") |> # ignore DNFs
  mutate(position = as.integer(position)) |>
  filter(position <= 15) |> # discard last 5
  group_by(year, round, circuitRef, constructorId) |>
...
Perhaps unsurprisingly, the gap between drivers of the same car is usually very low. Note that we are discarding the last 5 finishers, which often includes cars that were wounded but finished the race for the sake of data gathering.

```{r}
#| label: tbl-position-gaps-medians
#| tbl-cap: Median finishing position gaps between teammates, top 15 finishers only.
modal <- function(x) {
  ux <- unique(x)
...
position_gaps |>
  group_by(circuitRef) |>
  summarise(median_gap = median(gap), mean_gap = mean(gap), modal_gap = modal(gap), n_races = n()) |>
  arrange(median_gap, mean_gap, modal_gap, desc(n_races)) |>
  head(10) |>
  kable()

# TODO - what's the probability of a 1-gap, 2-gap, 3-gap etc.?
```
