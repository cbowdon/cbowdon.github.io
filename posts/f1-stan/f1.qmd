---
title: "Latent F1 driver and constructor performance in 2024: fun with Bayesian models"
author: "Chris Bowdon"
date: "2024-12-27"
...
For a bit of fun, let's try and model the performance of F1 drivers and constructors using Bayesian models. The aim is to more to learn about using [Stan](https://mc-stan.org) than determine deep truths about F1, since there's far more to performance than we can easily model.

We'll vaguely imitate the f1-metrics model: there are latent variables of driver skill and car quality, which predict points/positions. However I never did get hold of the paper for that, and am instead following the amazing [Rasmus Bååth's blog](https://www.sumsar.net/blog/2013/07/modeling-match-results-in-la-liga-part-one/) where he models the latent skill of football teams in La Liga.
...
At this point I'm convinced enough that Suzuka is a clear indicator of relative car performance, so let's take the position of each team's fastest car round Suzuka as constructor performance priors. Luckily there weren't any double-DNFs - at least one car from each team finished - so every team is represented.

The Stan model is the same, we're just injecting different prior mus and SDs.

```{r}
#| label: fig-suzuka-priors
...
Most teams didn't move far from their priors. I'd draw similar conclusions to before: the McLaren was fastest, Red Bull was a close second, and Ferrari were close behind.
...
There are some crucial changes. First to ensure the model is identifiable, the raw skill parameters are constrained to [0, 1] with an inverse logistic transform. Second, I've tried to arrange it so that the skill parameters are interpretable in terms of positions gained/lost. The idea is that a negative skill for a car/driver means it is more likely to finish in a lower position (i.e. closer to first place).

```{r}
#| label: run-drv-ctr-model
run_stan2 <- function(data_list, model_file = "f1.c.stan") {
  mod <- cmdstan_model(model_file, exe_file = str_c(model_file, ".bin"))
  mod$sample(data_list, seed = 42, show_messages = TRUE, parallel_chains = 8)
}
...
sample_posterior2 <- function(fit) {
  ...
  df_ctrs <- fit$draws("lambda_ctrs_raw", format = "draws_matrix") |> as_tibble()
  colnames(df_ctrs) <- ctrs

  df_drvs <- fit$draws("lambda_drvs_raw", format = "draws_matrix") |> as_tibble()
  ...
  list(
    ...
    drvs = df_drvs |>
      pivot_longer(
        all_of(drvs),
        names_to = "driverRef", values_to = "position"
      ) |>
      mutate(position = as.double(position))
  )
}
...
```{r}
#| label: fig-ctr-perf
#| fig-cap: Constructor performance parameters determined by model. Lower is better.
...
The model suggests - perhaps controversially - that the Red Bull was the fastest car, followed by Ferrari and _then_ the McLaren. It suggests that being in one of those three, Mercedes, or Aston Martin, would gain you positions relative to the field and anything else you would lose positions, which does seem reasonable.

Forgive me for not labelling the drivers on this next plot, but it's usually obvious which is which.

```{r}
#| label: fig-drv-perf
#| fig-cap: Driver performance parameters determined by model. Lower is better.
posterior_drvs <- sample_posterior2(fit.c)$drvs |> merge(ctr_drvs)
...
The model suggests that the top drivers performed similarly, with Verstappen fastest. Perez isn't considered so slow: but this could be because I removed all DNFs, flattering Perez.
