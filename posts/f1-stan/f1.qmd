...
posterior_drvs <- sample_posterior2(fit.c)$drvs |>#| 
  merge(ctr_drvs) |>
  group_by(driverRef) |>
  mutate(x = mean(position))

ggplot(posterior_drvs) +
  aes(x = position, fill = driverRef, label = driverRef) +
  facet_wrap(~constructorRef) +
  scale_fill_manual(values = drv_colours$colour) +
...
Intriguingly, the model considers that Sauber was faster than its competitors but let down by its drivers.
