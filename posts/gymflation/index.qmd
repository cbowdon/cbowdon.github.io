...
In recent years, Gym culture has exploded.
...
It's really handy to set up a caching mechanism. You could do this with a memoized function in code, or with a proxy. The proxy is a little easier because we can still use Ellmer's parallelisation functions easily. I have a little vibe-coded proxy that runs with Deno [here](./caching_proxy.ts). Note that most proxies won't work out-of-the-box, because chat completions are HTTP POST requests and these are not supposed to be cached.
...
sample.df <- df |> slice_sample(n = 2000)
...
It's readily apparent that the average weight lifted hasn't meaningfully changed. We can also inspect whether the absolute number of heavy lifts has changed with a tile plot.

```{r}
#| label: fig-tiles
#| fig-cap: In recent years there have been more videos posted with heavy weights, in absolute terms.
bins <- seq(0, 500, 25)

index <- expand.grid(
  year = min(df.weights$year):max(df.weights$year),
  weight.kg.1rm.bin = bins
)

df.weights.binned <- df.weights |>
  group_by(
    year,
    weight.kg.1rm.bin = cut(
      weight.kg.1rm,
      breaks = bins,
...
index |>
  ...
  ggplot() +
  aes(x = year, y = weight.kg.1rm.bin, fill = n.posts) +
  geom_tile() +
...
This is imperfect, because not ever video of a sumo deadlift will include the word "sumo". Those with "sumo" in the title or description are highly likely to be true positives, but not vice versa. Therefore we're comparing one precise distribution to a mixed distribution.
...
predict_gender_chat <- {
  ...
  Respond with `M` if the person appears male, `F` if the person appears female, or `NA` if it's impossible to tell.
Return your answer in a JSON object with a single key `gender` with value `M`, `F` or `NA`."

  chat_openai_compatible(
    LLM_SERVER_URL,
    #model = "mistralai/ministral-3-3b",
    model = "qwen/qwen3.5-35b-a3b", # ths larger model does worse because typing isn't allowing it to think
...
run_gender <- function(image.paths) {
...
df.wb <- df.downloaded.only |>
  select(id.videoId, image.path, title, description) |>
  mutate(gender = NA, reviewed = F)
...
writeData(wb, sheet = 1, "image", startCol = img.col)
for (i in seq_len(nrow(df.wb))) {
  insertImage(
    wb,
    sheet = 1,
    file = df.wb$image.path[i],
    startRow = i + 1,
    startCol = img.col,
    width = 1024,
    height = 1024,
...
Then we eval our predictions against the annotated data. The annotations are almost entirely male though.
...
gender.preds <- run_gender(df.downloaded.only$image.path)

img.eval <- bind_cols(
  rename(img.ann, gender.true = gender),
...
cm2 <- confusionMatrix(
  img.eval$gender.pred,
  reference = img.eval$gender.true,
  mode = "prec_recall"
)
cm2$table
