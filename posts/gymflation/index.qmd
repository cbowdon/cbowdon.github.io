...
Now we just run through another cycle of evaluation and ICL.
...
writeData(
  wb,
  sheet = 1,
  x = select(df.downloaded.only, id.videoId, title, description)
)
...
for (i in seq_len(nrow(df.downloaded.only))) {
  ...
  addImage(
    wb,
    sheet = 1,
    img = df.downloaded.only$image.path[i],
    startRow = i + 2,
    startCol = 1,
    width = 30,
    height = 25
...
gender.preds <- parallel_chat_structured(
  chat = predict_gender_chat,
  prompts = map(
    pull(df.downloaded.only, image.path),
    content_image_file
  ),
  type = predict_gender_type,
  max_active = 5
