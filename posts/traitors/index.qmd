...
sim_games <- function(..., .n_games = 1000) {
  1:.n_games |>
...
```{r}
results <- sim_games(22, 3, .n_games = 10000)
...
To avoid being too obvious, the traitors could split their votes: the majority target one faithful, and the remainder vote for random other players (including other traitors, to avoid inadvertently revealing them). We could assume the traitors were competent enough to rotate the majority members.

```{r}
subtle_ganging_up <- function(n_players, n_traitors) {
