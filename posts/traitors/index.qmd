...
crimson <- rgb(89 / 255, 14 / 255, 5 / 255)
```
...
Now the game is perfectly balanced.
...
That has a useful implication for the producers of the Traitors: if they want to keep the game balanced, the timing of adding new players is crucial.
...
It looks to me like the faithful do herd together to whomp one or two unlucky players. It's obviously not a pre-arranged strategy, just groupthink, but all the same quite likely.

Let's model the way the faithful play. We'll take the mean of the vote distributions as the probability of voting for target one, two, three, etc.
...
We'll take the mean normalised vote counts as a multinomial distribution, so that our simulation reflects actual vote distributions.
...
This isn't much worse than the fully random strategy, though it still gives the traitors a slight advantage. Could the faithfuls do better? Well yes, if they were any good at spotting traitors.

# Smart faithfuls

Let's grant the faithfuls a glimmer of intuition: they are twice as suspicious of traitors as faithfuls, i.e. a traitor is twice as likely to be the vote target as a faithful. We'll call that the _suspcicion_ factor.

```{r}
intuitive_herd <- function(n_players, n_traitors, suspicion_factor = 2) {
  intuition <- rep(1, n_players)
  intuition[1:n_traitors] <- suspicion_factor
  intuition <- intuition / sum(intuition)
  shuffled <- sample(n_players, n_players, prob = intuition)
  sample(shuffled, size = n_players - n_traitors, replace = TRUE, prob = vote_prob[1:n_players])
}

results <- sim_games(n_players = 22, n_traitors = 3, traitor_strategy = random_traitors, faithful_strategy = intuitive_herd)
plot_game_results(results, 22, 3)
```

That tips the balance resoundingly in favour of the faithfuls, showing how fragile the traitors' position really is.

```{r}
#| label: fig-suspicion
#| fig-cap: Traitor win probability as a function of suspicion.
suspicion <- c(0.5, 1.0, 1.2, 1.5, 2.0, 5.0, 10.0)
s_df <- suspicion |>
  map(
    \(s)
    sim_games(
      ...
      n_traitors = 3,
      traitor_strategy = random_traitors,
      faithful_strategy = function(...) intuitive_herd(..., suspicion_factor = s)
    ) |>
      mutate(suspicion = s),
    .progress = TRUE
  ) |>
