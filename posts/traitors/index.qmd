...
game2 <- function(
    n_players = 6,
    n_traitors = 2,
    traitors_influence = identity,
    faithfuls_influence = identity,
    traitors_strategy = random_traitors,
...
random_faithful <- function(suspicions, n_players, n_traitors) {
  # vote for a single random player
  target <- sample(1:n_players, size = 1)
  rep(target, n_players - n_traitors)
}

random_traitors <- function(suspicions, n_players, n_traitors) {
...
results <- sim_games(n_players = 22, n_traitors = 3, .game = game2)
plot_game_results(results, 22, 3)
```
