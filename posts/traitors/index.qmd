...
The minimum players for Werewolf (the game on which Traitors is based) is six, with two wolves and four villagers. Let's model that and build up. We'll set some basic default strategies for the traitors and the faithful.
...
game <- function(
  ...
  while (
    ...
    n_players <- n_players - 2 # EXILE! MURDER!
  }
...
That's resoundingly in favour of the traitors, and the picture isn't so different when we change the balance to be the same as the show.
...
```{r}
subtle_ganging_up <- function(n_players, n_traitors, vote_prob = 0.67) {
...
```{r}
results <- sim_games(n_players = 22, n_traitors = 3, traitor_strategy = subtle_ganging_up, faithful_strategy = rising_accuracy)
plot_game_results(results, 22, 3)
```

What can we do to make this more realistic though?

# What are actual vote distributions like?
...
votes <- list(
  ...
  lapply(\(v) {
    round_votes <- rep(0, sum(v))
    round_votes[1:length(v)] <- v
    round_votes
  })
```
