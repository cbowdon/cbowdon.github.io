...
unique_articles <- unique(samples_per_reader$article_ids)
```
...
simulate_responses <- function(unique_articles, sim_ident) {
  datafile <- sprintf("data/article-responses - %s.rds", sim_ident)

  if (!file.exists(datafile)) {
    process_reader <- function(art_ids) {
      arts <- upf_articles |> filter(article_id %in% art_ids)
      dist <- llm_answer_distribution(
        map(arts, as.list),
        simulated_identity = sim_ident
      )
      dist |>
        mutate(article_ids = list(art_ids), simulated_identity = sim_ident)
    }

    with(plan(multisession, workers = 4), {
      tasks <- future_map(unique_articles, process_reader, .progress = TRUE)
    })
...
article_responses <- simulate_responses(unique_articles, sim_ident)

article_responses |>
  group_by(article_ids) |>
