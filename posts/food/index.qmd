...
llm_answer_distribution <- function(
  ...
  # Invite the LLM to hallucinate a bit, to express its identity.
  # TODO - this is a mistake, it affects the later probability distribution. A fixed expression per identity would be better.
  thoughts <- chat$chat(
    sprintf(
      "What are thoughts on the following headline given your own views and experiences? Please provide one or two sentences.
  ...
  response <- chat$chat_structured(
    "Considering your previous response, answer this question.
  At the moment, how concerned, if at all, do you personally feel about ultra-processed, or over-processing of food?
...
sim_ident <- "Extremely chill dude"
  ...
  tasks <- future_map(
    ...
    function(row) {
      dist <- llm_answer_distribution(
        headline = row$title,
        source = row$domain,
        date = row$harvestTimeLong,
        simulated_identity = sim_ident
      )
      dist |>
        mutate(article_id = row$article_id, simulated_identity = sim_ident)
    }
  )
})
...
Now we have all the tools we need to do some very interesting work.

TODO - thought is making the probs unstable
