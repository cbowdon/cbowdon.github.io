...
To avoid sharing the licensed news data with a third-party, the model is a local LLM running with a patched version of mlx-omni-server.
...
ask(create_respondent("Average British person"), upf_question, RESPONSE_CATS) |>
  kable(digits = 3)
```
...
Answer the question, recalling that you have seen the following articles that may or may not have influenced your opinion.
...
```{r}
#| label: tbl-concerned
#| tbl-cap: Response distribution of LLM again, this time with the identity of someone very concerned about living a healthy lifestyle.
ask(
  create_respondent("Someone very concerned about living a healthy lifestyle"),
  question_with_articles(upf_question, fake_articles),
  RESPONSE_CATS
) |>
  kable(digits = 3)
...
#| tbl-cap: Response distribution of LLM for two different genders.
...
Some bias there! But to be brutally honest, that's kind of the point of AI simulated polling, to exploit the biases to model "typical" responses. If we look at the actual responses by gender, we can see that there really is a gender difference in the direction the model predicts i.e. female respondents were more likely to be highly concerned compared to males.
...
To make a useful gender comparison, we'd need to calibrate the LLM such that it's predictions are consistent with known gender biases. One way to achieve this would be reshaping the response distribution. We could also generate a sample of expected responses with the desired distribution and fine-tune the LLM on this.
...
simulate_responses <- function(unique_samples, sim_ident, force = FALSE) {
  datafile <- sprintf("data/article-responses - %s.rds", sim_ident)

  if (!file.exists(datafile) || force) {
    process_reader <- function(row) {
...
article_responses <- simulate_responses(
  unique_samples,
  sim_ident
)
...
ggplot(
  ...
  aes(x = period, y = Probability, group = Response, colour = Response) +
  geom_point() +
  stat_smooth(method = "lm")
```
