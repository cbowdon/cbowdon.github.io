...
This is a mixed bag. The trend in net concern does align, although it's clearly miscalibrated. The simulated response breakdown contradicts the actual response breakdown, because it shows opposing trends for "Highly" and "Somewhat" concerned.

## Digging deeper

If we look at the distribution of responses, we notice that almost all articles generate a "Somewhat concerned" response. 
...
ggplot(relfreq) +
  ...
  geom_col() +
  labs(x="Relative frequency of ultra-processed food articles among source's output", y=NULL)
```
...
n_s <- rmultinom(1, n, prob = readership$Readers)
...
# TODO need to establish a domain sort order
rbinom(1, size=n_s[[1]], prob=relfreq$Relative.Frequency[[1]] / max(relfreq$Relative.Frequency))

# Number of relevant articles
sample(
  articles |> filter(Domain == "bbc.co.uk"),
  size = n_s[[1]],
  replace = FALSE
)
```
