...
# that the expected number of BBC articles is 10 per month.
readership <- readership |>
  mutate(
    lambda = 10 * Readers / max(Readers),
    # TODO prob_upf
    prob_upf = 0.1
  )
...
result <- matrix(
  nrow = nrow(readership),
  ncol = n_sim_readers,
  dimnames = list(readership$Domain, 1:n_sim_readers)
)
for (i in 1:nrow(readership)) {
  row <- as.list(readership[i, ])
  n <- rpois(n_sim_readers, row$lambda * row$prob_upf)
  result[i, ] <- n
}
result
```
