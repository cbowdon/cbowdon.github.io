...
Note that I have merged the two categories "I don't know enough to comment" and "Don't know" in the original data for two reasons: first, there's a technical limit of 5 categories coming later, and second there is very little difference.
...
An idea that's generating a lot of excitement recently is the suggestion that you can simulate public opinion polls with AI. The concept is very simple: you calibrate multiple LLM assistants to represent your different demographics and then sample their responses to questions.
...
To move past this, we're going to have to build a model with some weighted contribution from each publisher each month.
...
domain_counts <- jsonlite::read_json(
  ...
  as_tibble() |>
  rename(Domain = key, nArticles = doc_count)

freqs <- articles |>
  group_by(Domain) |>
  count() |>
  merge(domain_counts) |>
  mutate(freq = n / nArticles) |>
  arrange(desc(freq))

abs_counts <- ggplot(freqs) +
  aes(y = forcats::fct_reorder(Domain, n), x = n, fill = Domain) +
  geom_col(show.legend = FALSE) +
  scale_fill_hue() +
  labs(x = "Number of articles on ultra-processed food", y = NULL)

all_articles <- ggplot(freqs) +
  aes(y = forcats::fct_reorder(Domain, n), x = nArticles, fill = Domain) +
  geom_col() +
  scale_fill_hue() +
  labs(x = "Number of articles (all topics)", y = NULL) +
  theme(
