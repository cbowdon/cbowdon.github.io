...
If we look at the distribution of responses, we notice that almost all articles generate a "Somewhat concerned" response. Now, we haven't calibrated the LLM on real human responses, but also if you just look at the headlines, anyone who takes them as fact most probably _would_ be concerned. Lack of calibration isn't our biggest problem here - the LLM is probably not far off reality - the problem is that media stories about ultra-processed food are overwhelmingly concerning.

So we're going to have to build a model with some weighted contribution from each publisher each month.
...
The only thing that would actually make a difference is how much the respondent trusted the headline, which itself is tightly coupled to which headlines the respondent is exposed to, because in practice people rarely read newspapers they don't trust.
