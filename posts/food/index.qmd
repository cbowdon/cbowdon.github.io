...
chat <- chat_openai(
  ...
  model = "gemma-3-1b-it",
  #model = "gpt-4.1-nano",
  system_prompt = "You are a member of the British public, with your own life experiences and views. Answer the questions from the user by first giving a sentence of reasoning and then your response.",
  params = params(log_probs = FALSE, seed = 42, n = 1)
)

chat$chat_structured(
  "Reflect on the following headline, and your own experiences, then answer the question.

  HEADLINE: Ultra-processed food linked to significant bowel cancer risk, scientists say. (Daily Mail, 12th Nov 2024)
  
  QUESTION: At the moment, how concerned, if at all, do you personally feel about ultra-processed, or over-processing of food?
    
  Give a sentence of reasoning and then decide your response.",
  type = type_object(
    reasoning = type_string(
      "One sentence of reasoning about how your personal experiences and views lead to your response."
    ),
    response = type_enum(
      description = "Following the reasoning, your response to the question: At the moment, how concerned, if at all, do you personally feel about ultra-processed, or over-processing of food?",
      values = c(
        "Highly concerned",
        "Somewhat concerned",
        "Not very concerned",
...
# map(chat$last_turn()@json$choices, \(x) x$message$content)
```

TODO the LLM is not reasoning properly, no connection between reasoning and response. Why?