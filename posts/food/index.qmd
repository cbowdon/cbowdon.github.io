...
extract_question <- function(file, question_prefix) {
  ...
  combined_headers[2] <- "Total(data) <- combined_headers
...
# TODO do we give their position prior? This would help with the tendency towards "somewhat".
# Or we could train the AI to be more emotive. Probably not.
# Maybe the AI needs to think more? But does that make it likely to equivocate?
# When I experimented with Qwen3 0.6B reasoning in chat, thinking didn't help.
# So maybe longitudinal studies are the way to go. We simulate the evolving opinion of each person.

# The problem that would most likely most improve results if solved, is this "somewhat" tendency.
# Second is a better media exposure model.
...
question_with_articles <- function(question, articles) {
  headlines <- articles |>
    map(function(a) {
      with(
        a,
...
ask(
  create_respondent(
    "Female who previously considered herself 'highly concerned' about UPFs."
  ),
...
Let's apply this to our articles now. We'll start with almost the dumbest possible demographic breakdown: readers are randomly assigned a gender.
...
map_simulated_readers <- function(
  unique_samples,
  ...
  dataset <- tibble(
    ...
    simulated_identity = character()
  )
  if (read_datafile && file.exists(datafile)) {
    dataset <- read_rds(datafile)
  }
  print(sprintf("Loaded %i rows from datafile", nrow(dataset)))

  samples_to_process <- unique_samples |>
    merge(dataset, all.x = TRUE) |>
    filter(is.na(Probability))

  if (nrow(samples_to_process) == 0) {
    print("All examples cached")
    return(dataset)
  }

  seq(1, nrow(samples_to_process), batch_size) |>
    reduce(
      function(ds, i) {
        ...
        print(sprintf(
          "Batch [%i, %i) of %i unprocessed samples",
          i,
          i + batch_size,
          nrow(samples_to_process)
        ))
        ...
        ds <- rows_upsert(
          ds,
          batch_responses,
          by = c("sample_key", "simulated_identity", "Response")
        )

        if (write_datafile) {
          write_rds(ds, datafile)
        }
        ds
      },
      .init = dataset
...
article_responses <- map_simulated_readers(
  ...
  mutate(Response = factor(Response, levels = RESPONSE_CATS, ordered = T)) |>
  rename(gender = simulated_identity)
...
for (i in seq_along(PERIODS)) {
  ...
  all_respondents <- genders |>
    merge(gender_resp |> rename(gender = simulated_identity)) |>
    select(reader, gender, Response, Probability) |>
    rows_update(
      readers,
      by = c("reader", "Response")
    )

  period_sim_dists[i, ] <- (all_respondents |>
    group_by(Response) |>
    summarise(p = mean(Probability)))$p
}
