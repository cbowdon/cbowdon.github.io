...
The UK population is approximately 70 million people. Given that the Guardian and the Daily Mail target very different demographics (left and right wing) we don't expect much overlap between their readership, i.e. their combined monthly online readership alone probably covers half the news-reading British population. Note also that BBC News isn't included, but has a similar level of traffic, albeit with higher overlap.
...
- Live television is a dying medium. (I was at an R meetup hosted by [Datacove](https://datacove.co.uk) recently when the session discussion turned to media channels. No one present had watched live TV in the last 24 hours, and only around 1 in 10 in the last month.)
...
article_counts <- bind_rows(
  jsonlite::read_json(
    ...
    as_tibble() |>
    mutate(mention = "focus", MentionType = "Focus articles"),
  jsonlite::read_json(
    "data/all-articles-counts.json",
    simplifyVector = T
  ) |>
    as_tibble() |>
    mutate(mention = "any", MentionType = "Articles with any mentions of UPFs")
) |>
  mutate(Period = as.Date(key_as_string)) |>
  filter(Period >= "2024-01-01" & Period < "2025-06-01")

ggplot(article_counts) +
...
The number of articles mentioning ultra-processed food is increasing over time. The fluctuation is most likely seasonal, and the trend is apparently upwards. There's _just_ about enough data to fix a SARIMA model.

```{r}
#| label: fig-sarima
#| fig-cap: A (very uncertain) SARIMA fit of the mention counts.
mention_counts <- ts(
  filter(article_counts, mention == "any")$doc_count,
  start = c(2024, 1),
  deltat = 1 / 12
)
...
However, there is no such trend in articles focused on ultra-processed food.

It's tempting to look at the upward trend in mentions, not the upward trend in net concern, and call it a day. Media mentions go up, net concern goes up, quod erat demonstratum! But no.

Because it's a bit of a flimsy conclusion, if you think about it. For one thing, news articles aren't fungible commodities! You can't swap any one for another and expect the same impact on public perception. An article that focuses on UPF ought to be considerably more impactful than one that mentions it in passing. We also know that some sources have multitudes more readers than others. We need to take a more nuanced approach.
...
Let's do the simplest thing possible: how does the trend in simulated responses compare to the trend in actual survey responses?
...
Let's therefore look at how the articles are distributed amongst the sources.
...
domain_counts <- jsonlite::read_json(
  "data/all-output-counts.json",
  simplifyVector = T
) |>
  as_tibble() |>
  rename(Domain = key, nTotal = doc_count)


art_freqs <- articles |>
  group_by(Domain) |>
  summarise(nArticle = n_distinct(ArticleId))

resp_freqs <- article_responses |>
  group_by(ArticleId) |>
  filter(Probability == max(Probability)) |>
  group_by(Domain, Response) |>
  summarise(nResponse = n_distinct(ArticleId))

freqs <- merge(art_freqs, resp_freqs) |> merge(domain_counts)

abs_counts <- ggplot(freqs) +
  aes(
    y = forcats::fct_reorder(Domain, nArticle),
    x = nResponse,
    fill = Response
  ) +
  geom_col() +
  labs(title = "Ultra-processed food", x = "Number of articles", y = NULL) +
  theme(legend.position = "bottom")

all_articles <- ggplot(freqs) +
  aes(y = forcats::fct_reorder(Domain, nArticle), x = nTotal) +
  geom_col() +
  labs(title = "All topics", x = "Number of articles", y = NULL) +
  theme(
    axis.text.x = element_text(angle = 45)
  )

abs_counts + all_articles
```

Oh, the Daily Mail has by far the most. Who'd have thought? Though in fairness it is a smaller proportion of their total output, which is enormous.

_(Incidentally, the Daily Mail also has the most articles about cancer, bone disease, and early death. I'm sure they employ a lot of medical professionals.)_

We now have enough information to build a probabilistic model of media exposure.

In a given period, a reader draws a count $n$ of articles they will read from a (non-negative) Poisson distribution. This represents the varying levels of news engagement.

$$
n ~ Pois(\lambda)
$$

We're assuming a discrete normal distribution of number of articles read. We'll look up a good value for $\lambda$ later.

They will then draw $n$ samples from a multinomial distribution of sources, where the multinomial probabilities are informed by the relative readership statistics. This gives us a vector of $n_s$, number of articles to read from each source.

$$
n_s ~ M(sources)
$$

Then we sample $n_s$ articles from each source. Each article $d$ has a probability $p$ of being about ultra-processed food, which is the relative frequency of articles about ultra-processed food from that source in that period.

If $d$ is about ultra-processed food, the reader's response $r$ to $d$ is drawn from the LLM's probability distribution of responses. Otherwise their response does not change.
...
```{r}
#| label: fig-relative-frequency
relfreq <- freqs |>
  group_by(Domain) |>
  summarise(Relative.Frequency = sum(nArticle) / max(nTotal)) |>
  arrange(Domain)

ggplot(relfreq) +
  aes(y = fct_reorder(Domain, Relative.Frequency), x = Relative.Frequency) +
  geom_col() +
  labs(
    x = "Relative frequency of ultra-processed food articles among source's output",
    y = NULL
  )
```

```{r}
#| label: fig-readership
...
readership <- tibble(
  ...
  mutate(Readers = Readers / sum(Readers))

ggplot(readership) +
  aes(y = fct_reorder(Domain, Readers), x = Readers) +
  geom_col() +
  labs(y = NULL, x = "Unique monthly readers (millions)")
```
...
# Number of articles in the time period
n <- rpois(num_sim_readers, lambda = 30) # approx. 1 article per day

# Number of articles per source
for (n_i in n) {
  n_s <- rmultinom(1, n_i, prob = readership$Readers)

  for (j in 1:length(DOMAINS)) {
    # We then have to do a Binomial test on each source given the relative frequency.
    # We normalise the relative frequency to avoid wasted samples.
    rbinom(
      1,
      n_s[[j]],
      prob = relfreq$Relative.Frequency / max(relfreq$Relative.Frequency)
    )
  }
}
...
#### BELOW THIS LINE IS OLD AND PROBABLY WRONG
