---
title: "Is experiencults the biggest predictor of F1 teammate battles?"
date: "2025-04-03"
categories: [r, f1]
warning: false
code-fold: true
...
Given that Lawson had just 11 races under his belt prior to this season, whereas Tsunoda has 90, I strongly suspect that he will. I _think_ that experiencults is at least as big a factor in driver performance as natural ability. Let's explore that!
...
There are three ways we could measure the experiencults of a driver: number of race starts, number of laps raced, and seasons endured.
...
Race starts is the most typical measure of driver experiencults. You might extend the idea of experiencults accumulating race by race to experiencults accumulating lap by lap, which would be particularly helpful to distinguish drivers in their earliest stages for whom every complete race is a significant chunk of experiencults. We might also consider whether the number of different cars raced helps, which is the same as the number of seasons except for those rare occasions when a driver jumps into two different cars in one seasons.
...
With each measure calculated for each driver for each race, we can look at how the difference in experiencults measure relates to teammate battle outcomes. For each pairing, for each race, we will calculate the finishing positions difference vs each experiencults measure.


```{r}
#| label: fig-scatter
#| fig-caption: Positions ahead of teammate against different measures of experiencults.
pairs <- driver.results |>
  filter(year >= 2010) |> # modern era
  group_by(year, round, constructorId) |>
  arrange(positionOrder) |>
  summarise(
    finished.ahead = first(positionOrder) < last(positionOrder),
    n.positions.ahead = last(positionOrder) - first(positionOrder),
...
ggplot(pairs |> pivot_longer(starts_with("n") & ends_with("diff"))) +
  aes(x = value, y = n.positions.ahead, group = name) +
...
ggplot(pairs |> mutate(n.starts.diff = scale(n.starts.diff))) +
  aes(x = n.starts.diff) +
  geom_histogram() +
  facet_wrap(~finished.ahead) +
  theme_minimal()
```

When plotted we can discern a slight apparent relationship. Let's try a logistic regression to get the odds ratios. There's no obvious difference between the different experience measures, so let's go with the conventional number of starts metric. We'll assume that it must have a nonlinear component and include the year and constructor as fixed effects. Sadly I can't separate out natural talent, but to an extent experience is a function of natural talent: those with more talent are generally welcome to drive for longer.

```{r}
model <- glm(
  finished.ahead ~ I(n.starts.diff^2) + factor(year) + factor(constructorId),
  family = "binomial",
  data = pairs, # mutate(pairs, n.starts.diff = scale(n.starts.diff)),
  control = list(maxit = 250)
)
```

If we compare that to a null model without the experience variable, we see it is significant.

```{r}
null.model <- glm(finished.ahead ~ factor(year) + factor(constructorId), family = "binomial", data = pairs)

anova(null.model, model)
```

We can interpret the model coefficients as an odds ratio.

```{r}
odds.ratio <- exp(coefficients(model)["I(n.starts.diff^2)"])

sprintf("Odds ratio: %s (i.e. every start increases odds of finishing ahead by %s%%)", round(odds.ratio, 3), round(100 * (odds.ratio - 1), 2))
```
